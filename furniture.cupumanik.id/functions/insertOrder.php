			...
			if ($res === true) {
				...
				if ($insertprodsuccess === true) {
					...
					if ($randomres === true) {
						...
						$mail->Subject = "Pemesanan Baru di Cupumanik Furniture";
						$mail->AddAddress ( "info@cupumanik.id", "Cupumanik Informasi" );
						$body = '<p>Kepada Yth. <strong>Cupumanik Administrator</strong>,</p>';
						$body .= '<p>Terdapat pemesanan baru untuk Cupumanik Furniture pada waktu: ' . date ( 'd F Y H:i:s' ) . '. Detail reservasi ditampilkan berikut ini:</p>';
						$body .= '<ul>';
						...
						$body .= '</ul>';
						$body .= '<p>Silakan membuka Cupumanik Furniture Administrator melalui tautan ini: <a href="http://furniture.cupumanik.id/admin">http://furniture.cupumanik.id/admin</a> untuk melihat detail pemesanan.</p>';
